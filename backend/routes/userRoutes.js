      ...
      User.findOneAndUpdate({_id: nuser._id}, {$set: { resetpass: true }}, function(err, doc) {
        if(err) {
          ...
          const content = {
            ...
            HTMLPart: getMailHtml(`${CLIENT_ORIGIN}/resetpass/${nuser._id}`, 'Reset your password'),
...
const getContactUsMailHtml = (name, email, message) => {
  ...
  <html lang="en">
    ...
    <body style="display: flex; background: black; color: white; text-align: center;">
      <div style="margin: auto; max-width: 600px;">
        <h1>RE-24 contact us</h1>
        <img src="https://re-24.com/assets/images/logo.png" style="max-width: 600px;" />
        <h5>Name: ${name}</h5>
        <h5>Email: ${email}</h5>
...
router.post('/contactusMessage', (req, res) => {
  const { name, email, message } = req.body;

  const content = {
    Subject: "Contact Us",
    HTMLPart: getContactUsMailHtml(name, email, message),
    CustomID: "CustomID"
  }

  sendEmail('hello@re-24.com', content).then(() => {
    res.json({
      success: true,
    });
  }).catch(err => {
